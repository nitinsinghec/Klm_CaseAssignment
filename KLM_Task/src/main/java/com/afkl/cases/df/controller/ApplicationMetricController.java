...
public class ApplicationMetricController {
	...
	Predicate<String> containStatus2XX = item -> item.contains(COUNTER_STATUS_2)
			&& !item.equals(COUNTER_STATUS_200_METRICS_ROOT);
	...
	Predicate<String> containStatus200 = item -> item.contains(COUNTER_STATUS)
			&& !item.equals(COUNTER_STATUS_200_METRICS_ROOT);
		...
		if (null != metricsMap && !metricsMap.isEmpty()) {
			...
			metricsMap.entrySet().forEach(entry -> {
				if (containStatus2XX.test(entry.getKey())) {
					lStatisticsVO.setStatus2xxRequestCount(lStatisticsVO.getStatus2xxRequestCount()
							+ Integer.valueOf(String.valueOf(entry.getValue())));
				} else if (containStatus4XX.test(entry.getKey())) {
					lStatisticsVO.setStatus4xxRequestCount(lStatisticsVO.getStatus4xxRequestCount()
							+ Integer.valueOf(String.valueOf(entry.getValue())));
				} else if (containStatus5XX.test(entry.getKey())) {
					lStatisticsVO.setStatus5xxRequestCount(lStatisticsVO.getStatus5xxRequestCount()
							+ Integer.valueOf(String.valueOf(entry.getValue())));
				}
				if (containStatus200.test(entry.getKey())) {
					lStatisticsVO.setTotalRequestCount(
							lStatisticsVO.getTotalRequestCount() + Integer.valueOf(String.valueOf(entry.getValue())));
				}
			});
		}
